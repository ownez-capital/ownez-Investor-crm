...
flowchart TD
  ...
  CO["Close-out prompt NEW<br/>⚠ Outstanding commitment shown<br/><br/>D Done · P Still pending · R Replace"]

  CO -->|D Done| FUL[Link activity to commitment<br/>Commitment ✓ fulfilled]
...
## 1.2 Terminology: internal vs user-facing

This distinction matters because you'll see both sets of terms when talking to Chad about the feature, but **only the internal set shows up in your Zoho work**.

| Concept | Internal / API / Zoho (your side) | User-facing / UI (what Chad sees) |
|---|---|---|
| The audit row | `Commitment Set` (Activity Type picklist value) | "Next action set" on the timeline |
| Fulfilled close-out | `fulfilled` (Commitment Status value) | "Done" (button + badge) |
| Replaced close-out | `superseded` (Commitment Status value) | "Replaced" (badge) |
| Cancelled close-out | `cancelled` (Commitment Status value) | "Cancelled" (badge) |
| Close-out hotkey | n/a | **D** (Done), P (Still pending), R (Replace) |

**What this means for you:** the Zoho schema uses `Commitment Set` / `fulfilled` / `superseded` / `cancelled` everywhere. These are the picklist values, API contract, and stored values. Do not rename them. The web app handles the translation to plain-English labels for Chad's screen. If anyone on your team reads the web UI or the Chad-facing user guide and sees "Done" or "replaced" or "Next action set," that's the same thing as "fulfilled" / "superseded" / "Commitment Set" in your world.
...
### 3.3 Workflow rules and custom functions that reference `Next Action Date`

If any Zoho workflow rule, custom function, or scheduled automation uses `Next Action Date` on the Person/Contact module as a trigger or filter, **audit it.** In most cases the right pivot is the same as §3.1: read from the Activity Log module instead, filtered by `Activity Type = Commitment Set AND Commitment Status = open AND Commitment Due Date <= today`.

Common places these lurk in Zoho deployments:
- **Workflow rules** on the Person module firing when `Next Action Date` changes — these no longer fire when a user just closes out a commitment (because the Person field isn't touched; only the Activity Log row's status changes). Rewrite them to trigger on the Activity Log module instead, filtered by `Activity Type = Commitment Set` and a change to `Commitment Status`.
- **Custom functions** aggregating "how many overdue this week" or similar stats — same Activity Log pivot as §3.1.
- **Scheduled reports** tied to `Next Action Date` — safer to rebuild against the Activity Log module for the same reason.

The Person-level `Next Action Date` is still written by the frontend as a denormalized convenience for Zoho native views. But it represents only the *current* commitment (the most recently created open one), not the full lifecycle. Do not use it as the source of truth for any automation that cares about "what's still open."

### 3.4 Webhooks subscribing to Activity Log writes

If any external integration (Make.com, Zapier, Slack notifications, reporting ETL, etc.) is subscribed to Activity Log creates in Zoho, it will start receiving rows with `Activity Type = Commitment Set` once the feature is live. These rows are system-generated by the frontend, not by Chad. If the subscriber treats every Activity Log create as "Chad did something new" and e.g. notifies a Slack channel, it will spam on every commitment set/change.

**Action for the Zoho team:** audit any webhook or external subscriber on the Activity Log module. Add a filter or conditional that excludes `Activity Type = Commitment Set` unless the subscriber explicitly cares about commitment audit events.
