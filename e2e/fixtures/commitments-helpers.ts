...
export async function logActivity(page: Page, personId: string, text: string) {
  ...
  // Wait for the async handleSubmit chain (POST /activities → GET
  // /commitments → state update) to settle. Without this, callers that use
  // non-waiting isVisible() checks immediately after logActivity can race
  // against the close-out prompt rendering. networkidle is a proxy for
  // "no more pending fetches"; after that settles the state update is
  // synchronous and the prompt (if any) is in the DOM.
  await page
